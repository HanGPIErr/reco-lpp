    ...
    public class AmbreDatabaseSynchronizer
    {
        ...
        private async Task ExecuteChangesAsync(ImportChanges changes, string countryId, bool assumeGlobalLockHeld, Action<string, int> progressCallback = null)
        {
            async Task ApplyChangesInternalAsync()
            {
                try
                {
                    ...
                    // Write sync marker so other users detect import changes within 3s
                    try
                    {
                        var dir = _offlineFirstService.GetParameter("CountryDatabaseDirectory");
        ...
        private ImportChanges CalculateChanges(List<DataAmbre> existingData, List<DataAmbre> newData)
        {
            ...
            foreach (var existingItem in existingData)
            {
                ...
                if (!newByKey.ContainsKey(key) && existingItem.DeleteDate == null)
                {
