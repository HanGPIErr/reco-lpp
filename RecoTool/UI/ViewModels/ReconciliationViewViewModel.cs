...
namespace RecoTool.UI.ViewModels
{
    ...
    public sealed class ReconciliationViewViewModel : INotifyPropertyChanged
    {
        ...
        {
            get => CurrentFilter.Category;
            set { if (CurrentFilter.Category != value) { CurrentFilter.Category = value; OnPropertyChanged(); } }
        }
        ...
        public List<ReconciliationViewData> ApplyFilters(IEnumerable<ReconciliationViewData> source, bool excludeTransactionType = false)
        {
            ...
            foreach (var x in source)
            {
                ...
                if (!string.IsNullOrWhiteSpace(f.Category) &&
                    (x.CategoryLabel ?? string.Empty).IndexOf(f.Category, System.StringComparison.OrdinalIgnoreCase) < 0)
                    continue;
                ...
                {
                    // 1️⃣ Découpage de la référence en plusieurs clés (séparateurs ; ou -)
                    var keys = dwRef
                    ...
                    // 2️⃣ Vérification OU sur l’ensemble des colonnes
                    anyMatch |= ContainsAny(x.DWINGS_InvoiceID, keys);
                    anyMatch |= ContainsAny(x.INVOICE_ID, keys);
                    anyMatch |= ContainsAny(x.DWINGS_GuaranteeID, keys);
                    anyMatch |= ContainsAny(x.GUARANTEE_ID, keys);
                    anyMatch |= ContainsAny(x.DWINGS_BGPMT, keys);
                    anyMatch |= ContainsAny(x.COMMISSION_ID, keys);
                    anyMatch |= ContainsAny(x.G_OFFICIALREF, keys);

                    if (!anyMatch) continue;          // aucune correspondance → on passe à l’item suivant
                }
        ...
        private bool ContainsAny(string source, string[] tokens)
        {
