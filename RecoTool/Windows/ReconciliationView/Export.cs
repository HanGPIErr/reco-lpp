...
namespace RecoTool.Windows
{
    ...
    public partial class ReconciliationView
    {
        ...
        /// <summary>
        /// Retourne true si la chaîne ne contient que des chiffres et dépasse 15 caractères
        /// (c’est exactement le scénario qui provoque la troncature d’Excel).
        /// </summary>
        private static bool IsLargeNumericString(string value)
        {
        ...
        /// <summary>
        /// Si la valeur est une chaîne qui représente un grand nombre, on la préfixe d’une apostrophe.
        /// Sinon on la retourne telle quelle.
        /// </summary>
        private static string ProtectLargeNumber(string value)
        {
            if (IsLargeNumericString(value) && !value.StartsWith("'"))
                return "'" + value;          // L’apostrophe n’apparaît pas dans la cellule Excel.
            return value;
        }
                              ...
                              RecoTool.Services.DTOs.ReconciliationViewData item)
        {
            try
            {
                ...
                // -------------------------------------------------
                // 2️⃣ Conversions spécifiques (votre logique d’origine)
                // -------------------------------------------------
                // Action, KPI, Incident Type, Reason Non Risky
                ...
                // To Remind / HasEmail
                if (header == "To Remind" || header == "HasEmail")
                {
                    if (raw == null) return string.Empty;
                    var bval = raw as bool?;
                ...
                switch (raw)
                {
                    ...
                    case bool boolVal:
                        ...
                        break;
                    case decimal dec:
                        result = dec.ToString("N2", CultureInfo.InvariantCulture);
                        break;
                    case double dbl:
                        result = dbl.ToString("N2", CultureInfo.InvariantCulture);
                        break;
                    default:
                        result = raw.ToString();
                        break;
                ...
                // -------------------------------------------------
                // 4️⃣ Protection des grands nombres sous forme de texte
                // -------------------------------------------------
                return ProtectLargeNumber(result);
            }
            catch
            {
                return string.Empty;
