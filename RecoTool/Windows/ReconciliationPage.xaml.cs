...
namespace RecoTool.Windows
{
    ...
    public partial class ReconciliationPage : UserControl, IRefreshable, INotifyPropertyChanged
    {
        ...
        // (multi-user badge timer removed for performance — check-on-open only)
        ...
        private async void ReconciliationPage_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Ensure tracker is ready for session registration when user opens a TodoList
                try
                {
        ...
        private async void AutoMatchButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Refresh ALL open views (not just page-level data)
                RefreshAllOpenViews();
        ...
        private async Task AddReconciliationView(bool asPopup = false)
        {
            ...
            // Wire DataChanged: when a view modifies data (bulk action, rules, etc.), refresh ALL sibling views
            view.DataChanged += () =>
            {
                try
                {
                    // Refresh all OTHER open views (the source view already refreshed itself)
                    ReconciliationService.InvalidateReconciliationViewCache(_currentCountryId);
                    _reconciliationService?.ClearViewCache();
                    foreach (var sibling in GetAllOpenViews())
                    {
                        if (sibling != view)
        ...
        #region Linking Basket

        private void OnAddToLinkingBasket(ReconciliationView sourceView, List<ReconciliationViewData> rows)
        {
            ...
            try
            {
                ...
                {
                    var side = row.AccountSide;
                    if (string.IsNullOrWhiteSpace(side) && country != null)
                    {
                        var acc = row.Account_ID?.Trim();
                        if (string.Equals(acc, country.CNT_AmbrePivot?.Trim(), StringComparison.OrdinalIgnoreCase))
                            side = "P";
                        else if (string.Equals(acc, country.CNT_AmbreReceivable?.Trim(), StringComparison.OrdinalIgnoreCase))
                            side = "R";
                    }
                    _linkingBasket.AddRow(row, side);
                }
        ...
        private void RefreshLinkingBasketUI()
        {
            try
            {
                ...
                if (_linkingBasket.CanLink)
                {
                    var delta = _linkingBasket.Delta;
                    if (_linkingBasket.IsBalanced)
                    {
                        LinkingBasketDelta.Text = "✅ Balanced";
                        LinkingBasketDelta.Foreground = System.Windows.Media.Brushes.DarkGreen;
                    }
                    else
                    {
                        LinkingBasketDelta.Text = $"⚠️ Δ {delta:N2}";
                        LinkingBasketDelta.Foreground = System.Windows.Media.Brushes.DarkOrange;
                    }
                    LinkBasketButton.IsEnabled = true;
                }
                else
                {
                    LinkingBasketDelta.Text = "Need P + R";
                    LinkingBasketDelta.Foreground = System.Windows.Media.Brushes.Gray;
                    LinkBasketButton.IsEnabled = false;
                }
            }
            catch { }
        }

        private async void LinkBasket_Click(object sender, RoutedEventArgs e)
        {
            if (!_linkingBasket.CanLink) return;
        ...
        /// <summary>
        /// Returns all open ReconciliationView instances from the ViewsPanel (searches visual tree of each container).
        /// </summary>
        private List<ReconciliationView> GetAllOpenViews()
        {
            var result = new List<ReconciliationView>();
            try
            {
                var panel = FindName("ViewsPanel") as StackPanel;
                ...
                {
                    if (child is ReconciliationView rv)
                    {
                        result.Add(rv);
                    }
                    else if (child is DependencyObject dep)
                    {
                        var found = FindReconciliationViewInTree(dep);
        ...
        /// <summary>
        /// Invalidates all caches and refreshes every open ReconciliationView.
        /// Call after any operation that modifies reconciliation data (bulk actions, linking, etc.).
        /// </summary>
        private void RefreshAllOpenViews()
        {
            try
            {
                ReconciliationService.InvalidateReconciliationViewCache(_currentCountryId);
                _reconciliationService?.ClearViewCache();
                foreach (var view in GetAllOpenViews())
                {
        ...
        private static ReconciliationView FindReconciliationViewInTree(DependencyObject parent)
        {
            if (parent is ReconciliationView rv) return rv;
            for (int i = 0; i < System.Windows.Media.VisualTreeHelper.GetChildrenCount(parent); i++)
            {
                var child = System.Windows.Media.VisualTreeHelper.GetChild(parent, i);
                var found = FindReconciliationViewInTree(child);
